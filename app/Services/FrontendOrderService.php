            ...
            DB::transaction(function () use ($request) {
                ...
                if (!blank($products)) {
                    foreach ($products as $product) {
                        ...
                        // get product model with seller
                        $productModel = Product::with('creator.seller')->find($product->product_id);

                        if ($productModel && $productModel->creator && $productModel->creator->seller) {

                            $seller = $productModel->creator;
                            $commission = $seller->seller->commission;

                            $sellingPrice = $product->price; // or $product->total

                            $commissionAmount = ($sellingPrice * $commission) / 100;
                            $balance = $sellingPrice - $commissionAmount;

                            // dd($balance);

                            $seller->increment('balance', $balance);
                            if($admin) {
                                $admin->increment('balance', $commissionAmount);
                            }

                        }else{
                            $admin->increment('balance', $product->price);
                        }
