...
class ProductService
{
    ...
    //         $query = Product::with('media', 'category', 'brand', 'taxes', 'tags', 'reviews', 'creator')
    //             ->with([
    //                 'wishlist' => fn($query) => $query->where(
    //                     'user_id',
    ...
    public function list(PaginateRequest $request)
{
    try {
        ...
        // Seller হলে status filter remove
        if (Auth::check() && Auth::user()->hasRole('Seller')) {
            unset($requests['status']);
        }
    ...
    public function productReport(PaginateRequest $request)
    {
        try {
            ...
            return Product::withCount('orders')->where(function ($query) use ($requests) {
                ...
                // Restrict for sellers: only their own products
                if (Auth::check() && Auth::user()->hasRole('Seller')) {
                    $query->where('creator_id', Auth::id())
                        ->where('creator_type', User::class);
                }
    ...
    public function productsReportOverview(Request $request)
    {
        try {
            ...
            $products = Product::withSum('productOrders', 'quantity')
