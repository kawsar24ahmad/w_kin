...
<div class="flex gap-3 mb-3">
    <button class="db-btn bg-red-500 text-white"
...
<div class="db-table-responsive">
    ...
    <label class="inline-flex items-center gap-2 cursor-pointer">
        <input
            ...
            v-model="allSelected"
            @change="selectAll"
            class=" text-purple-600 bg-gray-100 border-gray-300 rounded focus:ring-2 focus:ring-purple-500"
        >
        <span class="text-sm font-medium">Select All</span>
                ...
                <th>{{ $t('label.buying_price') }}</th>
                <th>{{ $t('label.selling_price') }}</th>
                <th>{{ $t('label.status') }}</th>
                <th>Added By</th>
                <th class="hidden-print" v-if="permissionChecker('products_show') || permissionChecker('products_edit') || permissionChecker('products_delete')">
                    {{ $t('label.action') }}
                </th>
            </tr>
        </thead>
        <tbody class="db-table-body" v-if="products.length > 0">
            <tr class="db-table-body-tr" v-for="product in products" :key="product.id">
                <td>
                    <input type="checkbox" :value="product.id" v-model="selectedProducts">
                </td>
                <td>{{ textShortener(product.name, 40) }}</td>
                <td>{{ product.category_name }}</td>
                <td>{{ product.flat_buying_price }}</td>
                <td>{{ product.flat_selling_price }}</td>
                <td><span :class="statusClass(product.status)">{{ enums.statusEnumArray[product.status] }}</span></td>
                <td>{{ product.company_name ? product.company_name : '-' }}</td>
                <td class="hidden-print" v-if="permissionChecker('products_show') || permissionChecker('products_edit') || permissionChecker('products_delete')">
                    <div class="flex gap-1.5">
                        <SmIconViewComponent :link="'admin.product.show'" :id="product.id" v-if="permissionChecker('products_show')" />
                        <SmIconSidebarModalEditComponent @click="edit(product)" v-if="permissionChecker('products_edit')" />
                        <SmIconDeleteComponent @click="destroy(product.id)" v-if="permissionChecker('products_delete')" />
                    </div>
                </td>
            </tr>
        </tbody>
...
export default {
        ...
        return {
            ...
            props: {
                form: {
                    ...
                    status:  this.authInfo?.seller_info?.role === 'seller'
                        ? statusEnum.INACTIVE
                        : statusEnum.ACTIVE,
    ...
    methods: {
        setStatus() {
            ...
            if (!this.authInfo?.seller_info?.role) return;

            const role = this.authInfo.seller_info.role;

            const status = role === 'seller'
                ? statusEnum.INACTIVE
                : statusEnum.ACTIVE;

            this.props.form.status = status;
         ...
         selectAll() {
          ...
          bulkDelete() {
            ...
            appService.destroyConfirmation().then(() => {
                ...
                this.$store.dispatch('product/bulkDestroy', { ids: this.selectedProducts, search: this.props.search })
                    .then(() => {
                        this.loading.isActive = false;
                        this.selectedProducts = [];
                        this.allSelected = false;
                        this.list(); // refresh product list
        ...
        clear: function () {
            ...
            this.props.search.status = this.authInfo?.seller_info?.role === 'seller'
                ? statusEnum.INACTIVE
                : null;
