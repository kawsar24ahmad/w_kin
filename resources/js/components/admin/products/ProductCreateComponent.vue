        ...
        <div class="drawer-body">
            <form @submit.prevent="save">
                <div class="form-row">
                    ...
                    <div class="form-col-12 sm:form-col-6">
                        <label for="selling_price" class="db-field-title required">
                            {{ $t("label.selling_price") }}
                             <small v-if="authInfo?.seller_info?.role == 'seller'">
                                Commission -{{ Number(authInfo?.seller_info?.commission) }}%
                            </small>
                        </label>
                    ...
                    <div v-if="authInfo?.seller_info?.role == 'seller'" class="form-col-12 sm:form-col-6">
                        <label class="db-field-title">
                            Seller Will Get
                        </label>
                        <input
                            type="text"
                            :value="sellerGetPrice"
                            readonly
                            class="db-field-control bg-gray-100">
                    </div>
        ...
        <div class="db-field-radio">
            <div class="custom-radio">
                <input type="radio" class="custom-radio-field"
                    v-model="props.form.status" id="inactive"
...
export default {
    ...
    computed: {
        ...
        sellerGetPrice() {
            if (
                this.authInfo?.seller_info?.role === "seller" &&
                this.props.form.selling_price
            ) {
                let price = Number(this.props.form.selling_price);
                let commission = Number(this.authInfo.seller_info.commission);

                let sellerGet = price - (price * commission / 100);

                return sellerGet.toFixed(2);
            }
