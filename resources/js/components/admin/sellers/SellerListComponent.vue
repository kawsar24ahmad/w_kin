    ...
    <div class="col-12">
        <div class="db-card">
            <div class="db-card-header border-none">
                <h3 class="db-card-title">{{ $t("menu.sellers") }}</h3>
                <div class="db-card-filter">
                    ...
                    <SellerCreateComponent :props="props" v-if="permissionChecker('sellers_create')" />
                </div>
            </div>
...
<button
    class="db-btn py-2 text-white bg-red-600 mb-3"
    @click="bulkDeleteEmployees"
    :disabled="selectedEmployees.length === 0"
>
    <i class="lab lab-line-trash lab-font-size-16"></i>
    <span>Delete Selected sellers</span>
</button>
    ...
    <label class="inline-flex items-center gap-2 cursor-pointer">
        <input
            ...
            class="w-5 h-5 text-purple-600 bg-gray-100 border-gray-300 rounded focus:ring-2 focus:ring-purple-500"
        >
        <span class="text-sm font-medium">Select All</span>
                            ...
                            <th class="db-table-head-th hidden-print"
                                v-if="permissionChecker('sellers_show') || permissionChecker('employees_edit') || permissionChecker('employees_delete')">
                                {{ $t("label.action") }}</th>
                        </tr>
                    </thead>
    ...
    <label class="inline-flex items-center gap-2 cursor-pointer">
        ...
        >
                           ...
                           <td class="db-table-body-td">{{ textShortener(employee.name, 20) }}</td>
                            ...
                            <td class="db-table-body-td">{{ employee.balance ?? 0 }}</td>
                            ...
                            <td class="db-table-body-td hidden-print"
                                ...
                                <div class="flex justify-start items-center sm:items-start sm:justify-start gap-1.5">
                                    <SmIconViewComponent :link="'admin.sellers.show'" :id="employee.id"
...
import SellerCreateComponent from "./SellerCreateComponent";
...
export default {
    ...
    computed: {
        ...
        employees: function () {
            return this.$store.getters["seller/lists"];
        },
        pagination: function () {
    ...
    bulkDeleteEmployees() {
        if (this.selectedEmployees.length === 0) {
            alertService.error(this.$t("message.select_at_least_one"));
            return;
        }

        appService
            .destroyConfirmation() // optional confirmation
            .then(() => {
                this.loading.isActive = true;
                this.$store
                    .dispatch("seller/bulkDelete", { ids: this.selectedEmployees })
                    .then(() => {
                        this.loading.isActive = false;
                        alertService.successFlip(null, this.$t("message.deleted_successfully"));
                        this.selectedEmployees = [];
        ...
        list: function (page = 1) {
            ...
            this.$store
                .dispatch("seller/lists", this.props.search)
                .then((res) => {
                    this.loading.isActive = false;
                })
                .catch((err) => {
        ...
        edit: function (employee) {
            ...
            this.loading.isActive = true;
            this.$store.dispatch("seller/edit", employee.id);
            this.loading.isActive = false;
            this.props.errors = {};
            this.props.form = {
                name: employee.name,
                ...
                company_name: employee?.seller_info?.company_name,
                category: employee?.seller_info?.category,
                commission: employee?.seller_info?.commission,
            };
