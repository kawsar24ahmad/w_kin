        ...
        <nav class="db-sidebar-nav">
            <ul class="db-sidebar-nav-list" v-if="menus.length > 0" v-for="menu in menus" :key="menu">
             ...
             <li class="db-sidebar-nav-item"  @click.prevent="sidebarActive($event)">
                    <router-link :to="'/admin/withdraw'" class="db-sidebar-nav-menu">
                        <i class="lab-fill-dollar-circle text-sm text-text cursor-pointer"></i>
                        <span class="text-base flex-auto">Withdraw</span>
                    </router-link>
                </li>
        </nav>
    </aside>
